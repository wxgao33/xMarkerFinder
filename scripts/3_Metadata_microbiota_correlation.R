...
phenotype_microbiota_corr<- function(metadata,microbiota){
    dist = vegdist(microbiota, method = 'bray', correction = 'lingoes', na.rm = TRUE)
    df_result <- matrix(nrow = 2, ncol = ncol(metadata),0)
    row.names(df_result) <- c('R2','Pvalue')
    colnames(df_result) <- colnames(metadata)
    for (i in 1:ncol(metadata)){
        tmp = metadata[,i]
        permanova = adonis(dist~tmp, permutations=999, method = "bray")
...
#permanova test
result <- phenotype_microbiota_corr(metadata,feat_abd)
write.table(result,file = paste(opts$workplace,opts$output,"_metadata_microbiota.txt",sep=''),sep = '\t')
